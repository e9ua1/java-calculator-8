...
- [x] 정적 팩토리 메서드 from()으로 String[] → Numbers 변환 책임 담당
...
### 4. DelimiterParser (전략 패턴 적용)

- [x] DelimiterParser 인터페이스 정의
- [x] CustomDelimiterParser: 커스텀 구분자 추출 및 파싱 통합 처리
    - [x] 문자열이 "//"로 시작하는지 확인
    - [x] "//"와 "\n" 사이의 문자를 커스텀 구분자로 추출
    - [x] 커스텀 구분자 이후의 숫자 부분만 추출
    - [x] 이스케이프된 개행문자(\\n) 처리 지원
    - [x] 매직 넘버 및 매직 스트링 상수화 (REAL_NEWLINE, ESCAPED_NEWLINE 등)
- [x] DefaultDelimiterParser: 기본 구분자 처리 및 상수화
- [x] FallbackParser: 단일 숫자 처리
- [x] 책임 연쇄 패턴으로 파서 선택

### 5. InputParser (파싱 책임 분리)

- [x] DelimiterParser들의 조합 로직 캡슐화
- [x] 입력 형식 예외를 Parser 레이어에서 처리
- [x] StringCalculator에서 파싱 책임 분리

### 6. StringCalculator (계산 흐름 조율)

- [x] null 또는 빈 문자열 입력 시 0 반환
- [x] InputParser로 파싱 책임 위임
- [x] Numbers.from()으로 변환 책임 위임
- [x] 계산 흐름 조율만 담당

### 7. CalculatorFactory (객체 조립)
...
├── Numbers.java                         # 숫자 컬렉션 (변환, 관리, 계산)
├── StringSplitter.java                  # 문자열 분리
└── parser/
    ├── InputParser.java                 # 파싱 책임 조합
    ├── DelimiterParser.java             # 파싱 전략 인터페이스
    ├── CustomDelimiterParser.java       # 커스텀 구분자 추출 및 파싱
    ├── DefaultDelimiterParser.java      # 기본 구분자 파싱
    └── FallbackParser.java              # 단일 숫자 처리
```
...
- 각 클래스는 하나의 책임만 가짐
- Number: 숫자 파싱과 검증
- Numbers: 숫자 변환, 컬렉션 관리 및 계산
- CustomDelimiterParser: 커스텀 구분자 추출 및 파싱 (높은 응집도)
- InputParser: 파싱 전략 조합
...
- **@DisplayName**: 한글로 테스트 의도를 명확히 표현
- **given-when-then 패턴**: 테스트 구조를 명확히 구분
- **assertAll**: 여러 검증을 동시에 실행하여 전체 실패 상황 파악
- **테스트 메서드명**: 영어로 작성하여 가독성 향상
